...
遵循[语义化版本](https://semver.org/lang/zh-CN/)：`主版本.次版本.修订号`（例如 `1.1.1`）。

| 什么时候 | 怎么加 | 例子 |
| --- | --- | --- |
| 只修 bug、调界面、改文档 | 修订号 +1 | `1.1.0` → `1.1.1` |
| 向下兼容地加功能 | 次版本 +1，修订号归零 | `1.1.1` → `1.2.0` |
| **行为不兼容**（默认值变了、JSON 字段改名、导出格式变了） | 主版本 +1 | `1.2.0` → `2.0.0` |

技术约束（这些是 .NET 定死的，写错会编译不过或属性页显示异常）：

- **程序集版本必须是 4 段数字**，每段 0–65534。代码里由 `AssemblyVersion4 = Const.Version + ".0"`
  自动补齐，别手写 4 段。
- 纯数字，**不能带字母**：`1.1.1-beta` 放不进程序集版本，要标预发布就写在 CHANGELOG 和 tag 里。
- 段值上限 65534，所以**别拿日期当版本号**（`20260919` 超范围）。

git tag 与校验：tag 写成 `v1.1.1`（比版本号多一个 `v`）。
发布流程会拿 tag 和 exe 自己报的 `--version` 比对，不一致直接失败。
