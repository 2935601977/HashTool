...
1. 把文件或文件夹**拖进窗口**，或者点「添加文件 / 添加文件夹」——也可以直接把文件/文件夹
   **拖到 `HashTool.exe` 图标上**，程序会启动并自动开始计算（等价于 `HashTool.exe D:\某个文件夹`）。
2. 等进度条走完。要重算点「重新计算全部」；文件有增删改点「刷新 (F5)」。
3. 选中行 → 右键复制，或 `Ctrl+C`，或点「导出 JSON」把整张表存下来。
